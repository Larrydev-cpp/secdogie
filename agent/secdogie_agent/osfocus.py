...
def match_windows(title: str, windows: list) -> list:
    """Rank windows matching `title`.

    `windows` is a list of objects with attributes:
      title (str), isVisible (bool, optional), isMinimized (bool, optional),
      width (int, optional), height (int, optional).

    Order: exact (case-insensitive) > prefix > substring. Within a tier,
    prefer visible, then non-minimized, then larger area. Empty title matches nothing.
    """
    ...
    substring: list = []
    for w in windows:
        wt = getattr(w, "title", None)
        if not isinstance(wt, str) or not wt:
            continue
        wt_l = wt.lower()
        if wt_l == needle_l:
            exact.append(w)
        elif wt_l.startswith(needle_l):
            prefix.append(w)
        elif needle_l in wt_l:
            substring.append(w)

    def _flag(obj, name: str, default):
    ...
    def _rank(w):
        visible = bool(_flag(w, "isVisible", True))
        minimized = bool(_flag(w, "isMinimized", False))
        try:
        ...
        try:
            ...
            height = 0
        return (visible, not minimized, width * height)

    def _sorted(group: list) -> list:
        return sorted(group, key=_rank, reverse=True)

    return _sorted(exact) + _sorted(prefix) + _sorted(substring)


def activate_title(title: str, *, settle_timeout: float = _DEFAULT_SETTLE_S, poll: float = _DEFAULT_POLL_S) -> bool:
    """Find a window matching `title` (exact, then prefix, then substring;
    case-insensitive) and force it frontmost (confirmed). For the single
    agent's --window targeting. False on Wayland (can't), if pywinctl is
    ...
    try:
        ...
        return False
    try:
        matches = match_windows(title, pywinctl.getAllWindows())
    except Exception:
        return False
    if not matches:
        return False
