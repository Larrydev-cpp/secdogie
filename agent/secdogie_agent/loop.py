...
def run(provider: VisionProvider, config: AgentConfig) -> int:
    ...
    try:
        for step in range(1, config.max_steps + 1):
            ...
            is_mutating = actions.is_mutating(action)
            # Pre-edit + high-risk: force confirmation even under --auto for any
            # action that can modify a document or permanently change files.
            force_confirm = (is_high_risk or is_mutating) and config.confirm_high_risk
            needs_confirm = action.kind not in _BENIGN and (not config.auto or force_confirm)
            if needs_confirm:
                if config.auto and force_confirm:
                    label = "HIGH-RISK" if is_high_risk else "DOCUMENT-MODIFYING"
                    logger.warning("%s action '%s' needs confirmation even under --auto", label, action.kind)
                label = "HIGH-RISK " if is_high_risk else ("DOCUMENT-MODIFYING " if is_mutating else "")
