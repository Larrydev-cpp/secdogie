...
class AgentConfig:
    ...
    # When True, a failed initial_focus or an action that ran with FOCUS_UNCONFIRMED_NOTE
    # aborts the run (exit 7) instead of warning and continuing. Auto-enabled by --window
    # so a pinned CAD/viewer session never acts on the wrong window.
...
def run(provider: VisionProvider, config: AgentConfig) -> int:
    ...
    # ghost of our own GUI. With require_focus the failure aborts (exit 7);
    # otherwise it is best-effort and the run continues with a warning.
    ...
    try:
        for step in range(1, config.max_steps + 1):
            ...
            # Risk gate: high-risk actions (open, run_elevated, Ctrl+S, Delete, Alt+F4, Ctrl+W)
            # still confirm under --auto unless confirm_high_risk was turned off.
