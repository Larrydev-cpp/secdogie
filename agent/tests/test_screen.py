...
def test_grid_overlay_still_valid_png_and_same_size():
    png = _png(1000, 800)
    out, (mw, mh), scale = screen.prepare_for_model(png, (1000, 800), max_edge=1568, grid=True, format="png")
    assert (mw, mh) == (1000, 800)
    # output remains a decodable image of the expected dimensions
    img = Image.open(io.BytesIO(out))
    assert img.size == (1000, 800)


def test_default_prepare_is_jpeg():
    png = _png(800, 600)
    out, size, scale = screen.prepare_for_model(png, (800, 600))
