...
# -- match_windows (tolerant --window ranking, no pywinctl needed) -------------


class _W:
    def __init__(self, title, visible=True, minimized=False, w=100, h=100):
        self.title = title
...
def test_match_windows_exact_beats_substring():
    wins = [_W("Notepad - notes.txt"), _W("Notepad")]
    # exact "Notepad" should win over the longer substring
    picked = osfocus.match_windows("Notepad", wins)
    assert picked[0].title == "Notepad"


def test_match_windows_prefix_when_no_exact():
    wins = [_W("Notepad - notes.txt"), _W("Other")]
    picked = osfocus.match_windows("Notepad", wins)
    assert picked[0].title == "Notepad - notes.txt"


def test_match_windows_case_insensitive():
    wins = [_W("NOTEPAD - notes.txt")]
    picked = osfocus.match_windows("notepad", wins)
    assert picked and picked[0].title == "NOTEPAD - notes.txt"


def test_match_windows_prefers_visible_non_minimized_larger():
    wins = [
        _W("App", visible=False, w=800, h=600),
        _W("App", visible=True, minimized=True, w=800, h=600),
        _W("App", visible=True, minimized=False, w=200, h=200),
        _W("App", visible=True, minimized=False, w=800, h=600),
    ]
    picked = osfocus.match_windows("App", wins)
    assert picked[0].width == 800 and picked[0].isVisible and not picked[0].isMinimized


def test_match_windows_empty_title_matches_nothing():
    assert osfocus.match_windows("", [_W("X")]) == []
    assert osfocus.match_windows("Nope", [_W("X")]) == []
