...
1. **主定位**走操作系统无障碍树（Windows UI Automation / AT-SPI / AX）——PID、hwnd、包围盒、AutomationId。用 `--desktop-ax` 打开。macOS 原生 MCT 抓**前台应用**的全部窗口（标题 / 描述 / 值 / 包围盒）；辅助功能未授权时回退 `CGWindowList`。AX 树就是触控板：`HitTest(x,y)` 落到最深控件再 `AXPress`。SIP 挡住 `task_for_pid` **不会**让 inspect 整单失败。实循环优先 `click_element` / 原生 Invoke；Darwin 上 `left_click` 也是命中测试 + AXPress，不是 HID。
