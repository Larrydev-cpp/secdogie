...
| [`platform/macos`](https://github.com/Larrydev-cpp/secdogie/tree/platform/macos) | macOS (Apple Silicon **and Intel x86_64**) | Frontmost / target pid AX tree (title, description, value, bounds) **is the pad**. `HitTest(x,y)` → deepest node → `AXPress`. `CGWindowList` names if Accessibility is not granted. SIP `task_for_pid` does not fail the inspect. CGWindow pixels are loop VERIFY only — inspect does not attach them as graphics. | `AXUIElementPerformAction(kAXPressAction)` only, including coordinate taps (hit-test). `CGEventPost` / IOHID / Quartz HID click is refused. Capture is `CGWindowListCreateImage` for pixel-diff verify, not a HID event tap. | `task_for_pid` + `mach_vm_region` + `mach_vm_read_overwrite` (optional; UI tree does not need it) | UTF-8 (CJK) then real UTF-16LE |
