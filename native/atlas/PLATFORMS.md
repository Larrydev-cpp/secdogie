...
| [`platform/macos`](https://github.com/Larrydev-cpp/secdogie/tree/platform/macos) | macOS (Apple Silicon **and Intel x86_64**) | Dual pad: AX tree (fine) + `AXUIElementCopyElementAtPosition` (OS finger). `HitTest` walks boxes if copy-at misses. Accessibility off → CGWindow **bounds** as coarse pad (no Screen Recording required). Titles + pixel-diff VERIFY need Screen Recording (`CGPreflightScreenCaptureAccess`). `grant` prompts TCC on the **host app**. SIP `task_for_pid` does not fail the inspect. | `AXUIElementPerformAction(kAXPressAction)` only, including coordinate taps. `CGEventPost` / IOHID / Quartz HID click is refused. Capture is `CGWindowListCreateImage` for pixel-diff verify, not a HID event tap. | `task_for_pid` + `mach_vm_region` + `mach_vm_read_overwrite` (optional; UI tree does not need it) | UTF-8 (CJK) then real UTF-16LE |
