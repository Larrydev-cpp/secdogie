...
| [`platform/macos`](https://github.com/Larrydev-cpp/secdogie/tree/platform/macos) | macOS (Apple Silicon **and Intel x86_64**) | Frontmost / target pid AX tree (title, description, value, bounds). `CGWindowList` if Accessibility is not granted. SIP `task_for_pid` does not fail the inspect. | `AXUIElementPerformAction(kAXPressAction)` only. `CGEventPost` / IOHID / Quartz HID click is refused. Capture is `CGWindowListCreateImage` (Screen Recording), not a HID event tap. | `task_for_pid` + `mach_vm_region` + `mach_vm_read_overwrite` (optional; UI tree does not need it) | UTF-8 (CJK) then real UTF-16LE |
